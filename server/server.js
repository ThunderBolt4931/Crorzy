...
app.post('/api/chat', requireAuth, upload.array('attachments', 5), async (req, res) => {
    try {
        ...
        const messages = [
            {
                role: "system",
                content: `You are a helpful AI assistant with access to Google Workspace services and file analysis tools. You have ${availableTools.length} tools available: ${availableTools.map(t => t.function.name).join(', ')}. When users upload files, analyze their content and provide helpful insights. Use the appropriate tools to help users with their Google Workspace tasks.`
            }
        ];
        ...
        if (response.tool_calls && response.tool_calls.length > 0) {
            ...
            // Track tokens from final completion
